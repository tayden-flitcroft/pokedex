...
export function parseSearch(params: URLSearchParams): SearchOptions {
  ...
  if (
    q.length > 80 ||
    (type && !isType(type)) ||
    (generation && !/^[1-9]$/.test(generation)) ||
    !['number', 'name', 'number-desc'].includes(sort) ||
    !/^\d+$/.test(page) ||
    Number(page) < 1 ||
    Number(page) > 10000 ||
    !/^\d+$/.test(limit) ||
    ...
    throw new SearchInputError(
      'Use a valid type, generation (1–9), sort, page (1–10000), and limit (1–48). Search is limited to 80 characters.',
    );
  return {
    q,
    type,
    ...
    .filter(
      (e) =>
        (!q ||
          ...
          (/^\d+$/.test(q) && e.id === Number(q))) &&
        (!range || (e.id >= range[0] && e.id <= range[1])) &&
        (!typeIds || typeIds.has(e.id)),
    )
    .sort((a, b) =>
      options.sort === 'name'
...
export async function searchPokemon(options: SearchOptions) {
  const [dex, type] = await Promise.all([
    getDex(),
    options.type
      ? pokeFetch<{ pokemon: { pokemon: Named }[] }>(`type/${options.type}`)
      : null,
  ]);
  const ids = type
    ? new Set(
        type.pokemon.map((p) =>
          Number(p.pokemon.url.split('/').filter(Boolean).at(-1)),
        ),
      )
    : undefined;
  ...
  const results = await Promise.all(
    matches
      .slice((page - 1) * options.limit, page * options.limit)
      .map(async (entry) => {
        const pokemon = await getDefaultPokemon(entry);
        // Cache only card data, not PokéAPI's large move and game-history payloads.
        return {
          id: pokemon.id,
          name: pokemon.name,
          dexName: entry.name,
