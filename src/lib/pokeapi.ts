...
export async function pokeFetch<T>(path: string): Promise<T> {
  ...
  try {
    ...
    throw new UpstreamError(
      'PokéAPI is taking a little too long to respond. Please try again.',
    );
  }
  if (response.status === 404)
    throw new MissingPokemonError('Pokémon not found');
  if (!response.ok)
    throw new UpstreamError(
      'PokéAPI is temporarily unavailable. Please try again.',
    );
  return response.json() as Promise<T>;
}
export const getDex = cache(async (): Promise<DexEntry[]> => {
  const dex = await pokeFetch<{
    pokemon_entries: { entry_number: number; pokemon_species: Named }[];
  }>('pokedex/national');
  return dex.pokemon_entries
    .map((p) => ({ id: p.entry_number, name: p.pokemon_species.name }))
    .sort((a, b) => a.id - b.id);
});
export const getPokemon = cache((name: string) =>
  pokeFetch<Pokemon>(`pokemon/${encodeURIComponent(name)}`),
);
export const getSpecies = cache((name: string) =>
  pokeFetch<Species>(`pokemon-species/${encodeURIComponent(name)}`),
);
...
export function number(id: number) {
  return `#${String(id).padStart(4, '0')}`;
}
export function artwork(p: Pick<Pokemon, 'sprites'>, shiny = false) {
  return (
    (shiny
      ? p.sprites.other['official-artwork'].front_shiny
      : p.sprites.other['official-artwork'].front_default) ||
    (shiny ? p.sprites.front_shiny : p.sprites.front_default) ||
    '/image-not-found.png'
  );
}
